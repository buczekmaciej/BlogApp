 ...
 */
final class Version20190705150715 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, login, password, email, joined_at FROM user');
        ...
        $this->addSql('INSERT INTO user (id, login, password, email, joined_at) SELECT id, login, password, email, joined_at FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, login, password, email, joined_at FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, login VARCHAR(15) NOT NULL, password VARCHAR(20) NOT NULL, email VARCHAR(125) NOT NULL, joined_at DATETIME DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, login, password, email, joined_at) SELECT id, login, password, email, joined_at FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
    }
}
