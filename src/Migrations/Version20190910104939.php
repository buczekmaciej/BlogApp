 ...
 */
final class Version20190910104939 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, details_id, login, password, email, joined_at FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, details_id INTEGER DEFAULT NULL, login VARCHAR(15) NOT NULL COLLATE BINARY, password VARCHAR(20) NOT NULL COLLATE BINARY, email VARCHAR(125) NOT NULL COLLATE BINARY, joined_at DATETIME DEFAULT NULL, is_disabled BOOLEAN DEFAULT NULL, CONSTRAINT FK_8D93D649BB1A0722 FOREIGN KEY (details_id) REFERENCES details (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO user (id, details_id, login, password, email, joined_at) SELECT id, details_id, login, password, email, joined_at FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, details_id, login, password, email, joined_at FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, details_id INTEGER DEFAULT NULL, login VARCHAR(15) NOT NULL, password VARCHAR(20) NOT NULL, email VARCHAR(125) NOT NULL, joined_at DATETIME DEFAULT NULL)');
        $this->addSql('INSERT INTO user (id, details_id, login, password, email, joined_at) SELECT id, details_id, login, password, email, joined_at FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_8D93D649BB1A0722 ON user (details_id)');
    }
